...
/**
 * Maps a browser location pathname/hash into one of the app's top-level routes.
 */
...
/**
 * Reads the serialized meeting route state from the current URL.
 */
...
/**
 * Builds a path-based URL for the target route and optional meeting state.
 */
...
/**
 * Lightweight client router that supports browser paths on web and hashes on desktop.
 */
  ...
  const value = useMemo<RouteContextValue>(() => {
    return {
      ...
      navigateToMeeting: (nextMeeting) => {
        ...
        // Meeting state is stored in the URL so a refresh can reconstruct the active room.
        setRoute("meeting");
